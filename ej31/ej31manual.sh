...
for usuario in "$@"; do
  usu=$(cut -d : -f1 /etc/passwd | grep "^$usuario$")
  if [ "$usuario" == "$usu" ]; then #verifico si el usuario existe
      home=$(grep "^$usuario:" /etc/passwd | cut -d : -f6)  #obtengo directorio personal de usuario 
